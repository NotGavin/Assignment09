...
# GChan, 2020-Mar-22, Created File
#------------------------------------------#
...
class Track():
    ...
    """
    # -- Constructor -- #
    def __init__(self, pos, ttl, leng):
        # -- Attributes -- #
        self.__position = pos
        self.__title = ttl
        self.__length = leng
    ...
    def position(self, value):
        if type(value) == int:
            if value < 1:
                raise Exception('Position cannot be 0 or negative')
    ...
    def length(self, value):
        if type(value) == str:
            ...
            raise Exception('Length must be a string (XX:XX)')
                
    # -- Methods -- #
    def __str__(self):
        """Returns Track details as formatted string"""
        return '{:>2}\t{} ({})'.format(self.position, self.title, self.length)  #copied from CD Methods
